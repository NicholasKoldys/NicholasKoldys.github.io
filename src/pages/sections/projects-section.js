...
export class ProjectsSection extends HTMLElement {
  ...
  connectedCallback() {
    this.shadowRoot.innerHTML = /* HTML */ `
      ...
      <project-card
        data-main-image-url="${matsoMain}"
        data-image-urls='["${matso1}", "${matso4}"]'
      >
        <h3 slot="title">${content.projects.matso.title}</h3>
        <p slot="description">
          ${content.projects.matso.description}<a
            slot="more"
            href="${content.projects.matso.gitPath}"
            >Learn More >></a
          >
        </p>
      </project-card>
      ...
      <project-card data-main-image-url="${noteApp}" data-image-urls="[]">
        ...
        <p slot="description">
          ${content.projects.collegeApp.description}<a
            slot="more"
            href="${content.projects.collegeApp.gitPath}"
            >Learn More >></a
          >
        </p>
      </project-card>
      ...
      <project-card
        data-main-image-url="${schedulerMain}"
        data-image-urls='["${scheduler2}", "${scheduler3}"]'
      >
        <h3 slot="title">${content.projects.scheduler.title}</h3>
        <p slot="description">
          ${content.projects.scheduler.description}<a
            slot="more"
            href="${content.projects.scheduler.gitPath} "
            >Learn More >></a
          >
        </p>
      </project-card>
      ...
      <project-card data-main-image-url="${inventory1}" data-image-urls="[]">
        <h3 slot="title">
          ${content.projects.inventoryManagementSystem.title}
        </h3>
        <p slot="description">
          ${content.projects.inventoryManagementSystem.description}<a
            slot="more"
            href="${content.projects.inventoryManagementSystem.gitPath} "
            >Learn More >></a
          >
        </p>
      </project-card>
